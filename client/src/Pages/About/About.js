import React, { Component } from 'react';



class About extends Component {




  // render nav
  render() {
    return (
      <div>


        <div id='wrapper1About'>
            <div className='uk-container'>
                <div className='' uk-grid='true'>
                    <div className='uk-width-1-1'>                        
                        <p className='uk-text-center uk-margin-large-top  aboutPara'>
                        Adam McMahan LMT has been practicing massage therapy most of his adult life. The creator of Surreality Massage LLC, 
                        he expressed passion early on in life for massage and the healing arts. Encouraged by many, he attended the Cortiva 
                        Institute of Massage Therapy in Chicago, IL just after Graduating High School in 2005. In 2006, he then embarked on 
                        a wide-reaching career of full-time massage, now expanding over 13 years and 25,000 sessions!                        
                        </p>
                        <p className='uk-text-center  aboutPara'> His knowledge extended in 2012, after receiving training and graduation from the Pima Medical Institute of Denver’s 
                        Physical Therapist Assistant program. Here, a new level in treatment developed with his approach, using massage therapy 
                        as well as concepts learned from his experience with physical therapy. 
                       </p>
                       <p className='uk-text-center  aboutPara'>
                       Gaining a high trust and comfort level, while providing only the best massage therapy service, every session, has been 
                        the main aspiration since day one. He has established a highly praised reputation with countless clients; including 
                        athletes, office workers, travelers, teachers, drivers, musicians, health practitioners, and literally all walks of life. 
                        In his free time, Adam likes to snowboard, play basketball, exercise at the gym, hike 14ers, practice Qi-gong, study 
                        astrology, enjoy live music, travel and spend time with his amazing friends and family.                       
                       </p>
                       <p className='uk-text-center  uk-margin-large-bottom  aboutPara'> Adam has been successfully treating clients and gaining great report since 2006 and is ready to offer you the best 
                        massage you have ever received. Book a session with him at Surreality Massage today!</p>
                    </div>
                </div>
            </div>
        </div>


      </div>
    );
  }
}


export default About;
